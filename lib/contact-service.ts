...
import { contactBlocked, contactStatusAfterAction, type ContactAction } from '@/lib/contact-policy';
...
export async function recordContactAction(input: {
  ...
  if (input.action === 'MARKED_SENT') {
    ...
    await updateLead(input.leadId, { contact_status: contactStatusAfterAction(lead.contact_status, input.action) });
  ...
  if (input.action === 'DO_NOT_CONTACT') {
    await updateLead(input.leadId, {
      contact_status: contactStatusAfterAction(lead.contact_status, input.action),
