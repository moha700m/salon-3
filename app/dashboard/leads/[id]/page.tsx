...
import { SalonCampaignPanel } from '@/components/salon-campaign-panel';
import { buildWhatsAppUrl } from '@/lib/phone';
import type { LeadRecord, OutreachMessageRecord, PreviewRecord, SalonCampaignRecord } from '@/types/domain';
...
interface DetailPayload { lead: LeadRecord; previews: PreviewWithUrl[]; messages: OutreachMessageRecord[]; contactLogs: ContactLog[]; campaign: SalonCampaignRecord | null }
...
export default function LeadDetailsPage() {
  ...
  function openWhatsApp() {
    ...
    const phone = parsedUrl.pathname.replace(/\D/g, '');
    const text = parsedUrl.searchParams.get('text') || '';
    const destination = isMobileDevice
      ? whatsappUrl
      : `https://web.whatsapp.com/send?phone=${encodeURIComponent(phone)}&text=${encodeURIComponent(text)}`;

    void fetch(`/api/leads/${id}/contact`, {
      method: 'POST',
  ...
  return (
    <main>
      ...
      <SalonCampaignPanel lead={lead} previewUrl={preview?.url} campaign={data.campaign} onReload={load} />
      ...
      <section className="mb-6 rounded-3xl border border-white/10 bg-white/[0.035] p-6">
        ...
        <textarea data-testid="outreach-message" dir="rtl" value={messageText} onChange={event => setMessageText(event.target.value)} rows={8} className="mt-5 w-full rounded-2xl border border-white/15 bg-black/60 p-4 leading-8 outline-none [overflow-wrap:anywhere] focus:border-yellow-500" placeholder={preview ? 'جاري تجهيز الرسالة...' : 'أنشئ المعاينة أولًا'} />
        <div className="mt-4 flex flex-wrap gap-3"><button onClick={saveMessage} disabled={!message || working === 'save-message'} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/15 px-4 py-3 font-bold disabled:opacity-40"><Save size={16} /> حفظ التعديل</button><button onClick={() => copy(messageText, 'تم نسخ الرسالة.')} disabled={!messageText} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/15 px-4 py-3 font-bold disabled:opacity-40"><Copy size={16} /> نسخ الرسالة</button><button onClick={openWhatsApp} disabled={!whatsappUrl || blocked || !messageText} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-green-500 px-5 py-3 font-black text-black disabled:opacity-40"><MessageCircle size={17} /> فتح واتساب</button></div>
